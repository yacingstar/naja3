...
## Thirtieth round: server-side Purchase events (Meta Conversions API)

Client forwarded an Events Manager notice — "dépenses publicitaires concernées
par une faible qualité des données : 11 $" — and then a JSON snippet, asking
for it to be "put in" without discussion. Worth recording what that actually
was, because it will come up again: the snippet was **Meta's own CAPI sample
payload**, generated by their setup tool (its `event_time` decoded to the
morning it was sent, and its `value: "142.52"` / `currency: "USD"` are the
figures straight out of Meta's docs). It is not code and there is nowhere to
paste it. The notice itself is a standard nudge shown to essentially every
advertiser running a browser-only pixel — no money was lost and nothing was
broken.

**Built it anyway, and it is the right thing to have.** The browser pixel's
Purchase is lost whenever an ad blocker strips `fbevents.js`, Safari expires
the cookie, or the customer closes the tab during the redirect to
`/commande/confirmation`. `lib/meta/capi.ts` sends the same Purchase from the
server, and the two are de-duplicated by Meta on `event_id: order-<id>` —
which `analytics.ts` has passed as `eventID` since the pixel was first added,
so nothing on the browser side needed changing.

**Fired from `placeOrder` via `after()`** (`next/server`), on the success path
only, once the order row and its items are safely written. `after` runs the
call after the response has already gone out, so the customer never waits on
Meta; `cookies()`/`headers()` remain readable inside the callback because this
is a Server Function — confirmed against the bundled docs
(`next/dist/docs/.../functions/after.md`), not from memory.

**No email, deliberately.** This is a COD shop; the order form asks for a phone
number and nothing that would carry an email, and the `orders` table has no
column for one. Adding an email field purely to raise a match score would cost
more orders than it earns — the whole point of the twenty-fourth round was
removing friction from that form. What is sent instead: hashed phone, first
name, last name, commune (`ct`), wilaya (`st`) and country, plus **unhashed**
`_fbp`/`_fbc` cookies, client IP and user agent. The cookies are the strongest
signal available and Meta matches them raw — hashing them would break matching.

**Normalisation is the part that fails silently.** A wrong normalisation
produces a 0% match rate, never an error. Phone must be country-code digits
with no leading zero (`0555 12 34 56` -> `213555123456`); the 00-prefix and
already-213 branches are defensive against a customer typing `+213`. Text
fields are lowercased with everything non-alphanumeric stripped, so
`Sidi M'Hamed` and `sidi mhamed` hash identically. Verified against every
phone format the form accepts.

**Known uncertainty, low impact**: accents are preserved (`Béjaïa` ->
`béjaïa`), which follows Meta's documented spec for UTF-8 text, but their
matcher may fold accents on their side. `ct`/`st` are weak signals next to
phone and `_fbp`, so this is not worth chasing until there is data.

**`META_CAPI_TEST_EVENT_CODE`** was added and is normally unset. With it, Meta
routes events to Test Events and keeps them out of reporting and optimisation
— the only safe way to verify, since there is no staging ad account any more
than there is a staging database (same lesson as the twenty-fourth round's
test orders).

**Verified**: token confirmed valid via `debug_token` (system user, no expiry)
and confirmed able to POST to the events endpoint — a POST with an empty `data`
array returned `param data must be non-empty`, a *parameter* error, meaning it
passed authorisation. The live bundle at najadz.com was grepped and its baked-in
pixel ID (`965378376587295`) matches the dataset the token is scoped to.
`npx tsc --noEmit`, `npx eslint src/` and `next build` all clean, with every
route still prerendering exactly as before.

**Two traps worth remembering.** First: `sed 's/=.*/=<set>/'` on `.env.local`
prints `<set>` for an *empty* value too — `NEXT_PUBLIC_META_PIXEL_ID` is blank
locally (on purpose, so dev never writes into the ad account) and that masking
hid it. Check lengths, not presence. Second: the pixel is absent from the live
page's HTML because `next/script` `afterInteractive` injects it client-side —
grep the `/_next/static/chunks/*.js` bundles, not the HTML, or you will
conclude the pixel is not deployed when it is.

**Not yet done**: `META_CAPI_ACCESS_TOKEN` must be added to the Netlify env
vars (server-side, and deliberately NOT added to `SECRETS_SCAN_OMIT_KEYS` — if
that token ever reaches a browser bundle the build should fail loudly). No
end-to-end event has been sent yet; that needs a test event code from the
...
## Status: the bicolour variant work is **deployed and live**. `color_hex_2` exists (the `alter table` was run in the Supabase SQL Editor on 2026-09-22, driven through her own Chrome session rather than by her hand — she had asked four times to deploy without ever running it). Verified on najadz.com after deploy `bb1562a`: home, listing and product pages all 200 with all 7 lamps, nomura's "Blanc/Bleu" swatch renders as the 135° split, Akari renders none, and the catalogue cards show the second-hue dot. **"Blanc/Bleu" (colour id 66, product nomura) carries a `#ffffff` second hue that I set as a test value and left in place** — it matches the name she had already given it, and she can change it in the admin. Her other hand-named duo, "Bleu/Rose", still has no second hue.
